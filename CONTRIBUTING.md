...
Releases are automated via [GoReleaser](https://goreleaser.com/) and [git-cliff](https://git-cliff.org/):

- **Stable releases** — when a tag matching `v*.*.*` is pushed, CI runs tests and lint, generates release notes with git-cliff, then builds binaries for all platforms (Linux, macOS, Windows × AMD64, ARM64) and publishes to GitHub Releases with Debian/RPM packages.
...
`CHANGELOG.md` is auto-generated by [git-cliff](https://git-cliff.org/) from conventional commit messages. The config is in `cliff.toml`. To regenerate locally (requires [git-cliff](https://git-cliff.org/docs/installation) to be installed):

```sh
make changelog
```

Contributors don't need to run GoReleaser or git-cliff locally. Just open a PR, get it merged, and a maintainer will tag the release.
